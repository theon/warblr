...
public class Clip {
    ...
    public static Clip newInstance(File file) throws UnsupportedAudioFileException, IOException {
        ...
        return new Clip(file.getName(), in, DEFAULT_FRAME_SIZE, DEFAULT_OVERLAP);
    }
...
//        logger.info(String.format("Read %d frames from %s (%d bytes). frameSize=%d overlap=%d\n", frames.size(), name, frames.size() * buf.length, frameSize, overlap));
     ...
     */
    public AudioInputStream getAudio(int sample, final int length) {
        ...
        InputStream audioData = new InputStream() {
            ...
            public int read() throws IOException {
                if((nextFrame - initialFrame)  == numOfFrames) {
                    return -1;
                }
    ...
    public int getClipLength(int frameNum) {
        return frameNum * getFrameTimeSamples() * (AUDIO_FORMAT.getSampleSizeInBits() / 8) / overlap;
    }

    public int getClipLength() {
        return getClipLength(getFrameCount());
    }
     ...
     */
    public Clip copy() {
        return this.subClip(0, getFrameCount(), getFrameFreqSamples(), getOverlap());
    }
